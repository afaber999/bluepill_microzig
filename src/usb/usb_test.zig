...
pub fn USBLIB_EPHandler(status: u32) void {
    ...
    var ep = EPRegs.get(ep_num);
    std.log.info("USBLIB_EPHandler({}) status: 0b{b:0>16} ep 0b{b:0>16}  FNR 0b{b:0>16}", .{ ep_num, @truncate(u16, status), @truncate(u16, ep), @truncate(u16, peripherals.USB.FNR.raw) });
    // dump_bdt_mem();
    //dump_pma_mem_range(0, 256);
