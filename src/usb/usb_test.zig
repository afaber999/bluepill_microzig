...
fn USB_EPHandler(status: u32) void {
    ...
    std.log.info("USB_EPHandler status: 0b{b:0>16} ep 0b{b:0>16}", .{ @truncate(u16, status), @truncate(u16, ep) });
    ...
    if ((ep & EPRegs.EP_CTR_RX_MASK) != 0) {
        ...
        var length = bdt.rx_cnt.data / 2;

        dump_pma_mem_range(start, length);
    }
...
fn usb_poll() void {
    while (true) {
        ...
        if (istr.SUSP != 0) {
            ...
            std.log.info("CTR !!! ISTR 0b{b:0>16} FNR 0b{b:0>16}", .{ @truncate(u16, peripherals.USB.ISTR.raw), @truncate(u16, peripherals.USB.FNR.raw) });
